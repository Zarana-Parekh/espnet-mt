...
expdir_main=/data/ASR5/zpp/espnet/egs/howto/exp
...
if [ ${stage} -le 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --debugmode ${debugmode} \
        --dict /tmp/zpp/data/mapping/dict2.txt \
        --debugdir ${expdir} \
        --minibatches ${N} \
        --verbose ${verbose} \
        --resume ${resume} \
        --seed ${seed} \
        --train-feat  /tmp/zpp/data/merge.json \
        --valid-feat /tmp/zpp/toy/merge.json \
        --train-label /tmp/zpp/data/merge.json \
        --valid-label /tmp/zpp/toy/merge.json \
        --etype ${etype} \
        --elayers ${elayers} \
        --eunits ${eunits} \
        --eprojs ${eprojs} \
