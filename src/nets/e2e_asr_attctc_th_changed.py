...
def pad_list(xs, pad_value=float("nan")):
    ...
    pad = Variable(xs[0].data.new(n_batch, max_len, *xs[0].size()[1:]).zero_() + 115)
    # removed pad_value
...
class E2E(torch.nn.Module):
    def __init__(self, idim, odim, args):
        ...
        self.embed = torch.nn.Embedding(embedding_dim=idim, num_embeddings=500, padding_idx=idim-1)
        # ctc
...
class Decoder(torch.nn.Module):
    def __init__(self, eprojs, odim, dlayers, dunits, sos, eos, att, verbose=0,
        ...
        self.embed = torch.nn.Embedding(num_embeddings=odim, embedding_dim=dunits, padding_idx=63499)
